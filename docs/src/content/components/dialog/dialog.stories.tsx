...
export const LandingPage = {
  render: () => (
    <div class="flex h-full items-center px-8 py-6">
      <div class="grid w-full content-start gap-3 bg-white">
        <div class="flex items-center justify-between gap-3">
          ...
          <Dialog label="Invite team">
            <div class="w-[22rem]">
              ...
              <p class="text-sm text-zinc-600">Add people before the review starts.</p>

              <div class="mt-4 grid gap-3">
                <div class="grid gap-1.5">
                  <label for="invite-emails" class="font-medium text-xs text-zinc-700">
                    Invite people
                  </label>
                  <Input
                    id="invite-emails"
                    ...
                    <a-popover class="group block w-full" placements="bottom-center">
                      <div class="group z-50 mt-1 block w-[350px] rounded-md border border-zinc-200 bg-white opacity-0 shadow-lg transition-opacity duration-100 group-[&[enabled]]:opacity-100">
                        <a-list class="group -translate-y-1 max-h-[200px] overflow-auto rounded-md bg-white transition-all duration-150 group-[&[enabled]]:translate-y-0">
                          {["Can edit", "Can comment", "View only"].map((item, index) => (
                            <a-list-item
                              key={index}
        ...
        <div class="grid grid-cols-3 content-start gap-2">
          {[
            ...
            <div key={name} class="grid gap-1 border border-zinc-200 bg-white px-3 py-2">
              <div class="grid grid-cols-[auto_1fr] items-center gap-2">
                <div class="h-7 w-7 flex-none rounded-full bg-zinc-200" />
                <div class="min-w-0">
                  <div class="truncate text-sm text-zinc-800">{name}</div>
                  <div class="truncate text-xs text-zinc-500">{role}</div>
                </div>
              </div>
