...
class Slider extends Dragger {
  ...
  needsControls () {
    if (this.options.mode === 'horizontal') {
      return this.options.center || (this.children.length > this.calcSlidesToShow() && this.element.clientWidth > this.options.container.clientWidth)
    } else {
      return this.options.center || (this.children.length > this.calcSlidesToShow() && this.element.clientHeight > this.options.container.clientHeight)
    }
  }
    ...
    if (this.options.arrows) {
      if (this.needsControls()) {
        if (!this.containerArrows) {
          const containerArrowsPlaceholder = this.options.container.parentNode.querySelectorAll('.slider__arrows')[0]
          ...
          const containerArrowPrevPlaceholder = this.options.container.parentNode.querySelectorAll('.slider__arrows-item--prev')[0]
          ...
          if (this.currentIndex === 0) this.containerArrowPrev.classList.add('slider__arrows-item--disabled')
          ...
          const containerArrowNextPlaceholder = this.options.container.parentNode.querySelectorAll('.slider__arrows-item--next')[0]
          ...
          if (this.currentIndex === this.children.length - this.calcSlidesToShow()) this.containerArrowNext.classList.add('slider__arrows-item--disabled')
          if (!containerArrowPrevPlaceholder) {
  ...
  calcSlidesToShow () {
    ...
    if (this.options.mode === 'horizontal') {
      return this.options.variableWidth ? this.options.center ? 1 : this.children.reduce((acc, child, index) => {
        acc.width += child.clientWidth + gap
        if (acc.width < this.options.container.clientWidth) acc.count = index + 1
        
        return acc
      }, { width: 0, count: 0 }).count : this.options.slidesToShow
    } else {
      return this.options.variableHeight ? this.options.center ? 1 : this.children.reduce((acc, child, index) => {
        acc.height += child.clientHeight + gap
        if (acc.height < this.options.container.clientHeight) acc.count = index + 1
        
        return acc
      }, { height: 0, count: 0 }).count : this.options.slidesToShow
    }
  }
    ...
    if (this.options.dots) {
      if (this.needsControls()) {
        ...
        if (!this.containerDots) {
          ...
          const containerDotsPlaceholder = this.options.container.parentNode.querySelectorAll('.slider__dots')[0]
          ...
          if (this.options.goByView && this.containerDotsItems.length !== (Math.floor((this.children.length - 1) / slidesToShow) + 1) || !this.options.goByView && this.containerDotsItems.length !== (this.children.length - slidesToShow) + 1) {
              ...
              switch (this.options.dots) {
                case 'simple':
                  this.children.forEach((child, index) => {
                    if (this.options.center || this.options.goByView && index % slidesToShow === 0 || !this.options.goByView && index <= this.children.length - slidesToShow) {
                ...
                case 'dragger':
                  ...
                  barDragger.style.width = (this.options.container.clientWidth / (this.children.length - slidesToShow + 1)) + 'px'
                  ...
                  class dotsDragger extends Dragger {
                    ...
                    calcSliderPosition () {
                      ...
                      return scale(percentageDraggerMoved, 0, 1, boundsSlider.boundLeft, boundsSlider.boundRight)
                    }
                    ...
                    onResize (event) {
                      ...
                      const boundsDragger = this.checkBounds()
                      this.drag.x = scale(this.context.currentIndex / (this.context.children.length - 1), 0, 1, boundsDragger.boundLeft, boundsDragger.boundRight)

                      this.translate(undefined, false)
                    }
                  }

                  this.dotsDragger = new dotsDragger(barDragger, {
                    autoResize: true,
                    lock: { y: true },
  ...
  setCurrentIndex (index) {
    ...
    this.currentIndex = Math.max(Math.min(index, this.children.length - (this.options.center ? 1 : slidesToShow)), 0)
    ...
    if (this.containerArrows) {
      ...
      const nextAction = this.containerArrowNext && this.currentIndex >= this.children.length - slidesToShow + (this.options.center ? slidesToShow - 1 : 0) ? 'add' : 'remove'
    ...
    if (this.containerDots && this.containerDotsItems.length) {
      const currentDots = this.containerDotsItems.filter(dotsItem => this.currentIndex >= Math.max(0, (parseInt(dotsItem.getAttribute('data-index')) + 1) - slidesToShow))
      const currentDot = this.currentIndex === 0 ? currentDots[0] : this.options.goByView ? currentDots[currentDots.length - 1] : this.containerDotsItems[this.currentIndex]
      ...
      if (currentDot && !currentDot.classList.contains('slider__dots-item--current')) currentDot.classList.add('slider__dots-item--current')
    }
    ...
    if (this.currentIndex !== currentIndexOld) {
      ...
      this.options.onChange && this.options.onChange(this, currentIndexOld, this.currentIndex)
    }
  }
  ...
  setClassesAndAttributes () {
    ...
    this.options.overflow ? this.options.container.parentNode.classList.add('slider--overflow') : this.options.container.parentNode.classList.remove('slider--overflow')
    this.options.rtl ? this.options.container.parentNode.classList.add('slider--rtl') : this.options.container.parentNode.classList.remove('slider--rtl')
    this.options.variableWidth ? this.options.container.parentNode.classList.add('slider--variable-width') : this.options.container.parentNode.classList.remove('slider--variable-width')
    this.options.variableHeight ? this.options.container.parentNode.classList.add('slider--variable-height') : this.options.container.parentNode.classList.remove('slider--variable-height')
    if (!this.options.center) this.options.centerSlides ? this.options.container.parentNode.classList.add('slider--center-slides') : this.options.container.parentNode.classList.remove('slider--center-slides')
  }
  ...
  updateOffsets () {
    if (this.options.mode === 'horizontal') {
      this.options = _.merge(this.options, {
        offsets: {
          left: this.options.center ? this.options.container.clientWidth / 2 - this.element.firstElementChild.clientWidth / 2 : 0, // breaks slider when node is #text so use firstElementChild
          right: this.options.center ? this.options.container.clientWidth / 2 - this.element.lastElementChild.clientWidth / 2 : 0, // breaks slider when node is #text so use lastElementChild
        }
      })
    } else {
      this.options = _.merge(this.options, {
        offsets: {
          top: this.options.center ? this.options.container.clientHeight / 2 - this.element.firstElementChild.clientHeight / 2 : 0, // breaks slider when node is #text so use firstElementChild
          bottom: this.options.center ? this.options.container.clientHeight / 2 - this.element.lastElementChild.clientHeight / 2 : 0, // breaks slider when node is #text so use lastElementChild
        }
      })
    }
  ...
  bindEvents () {
    ...
    Array.from(this.element.querySelectorAll('a, :scope > *')).forEach(link => link.addEventListener('click', this.boundedOnClickFn, true))
  ...
  unbindEvents () {
    ...
    Array.from(this.element.querySelectorAll('a, :scope > *')).forEach(link => link.removeEventListener('click', this.boundedOnClickFn))
  ...
  getPostitionByChild (child) {
    if (child) {
      ...
      if (this.options.rtl) {
        const offsetX = this.element.clientWidth - this.options.container.clientWidth - child.offsetLeft
  ...
  goToIndex (index, duration) {
    if (index !== undefined && !isNaN(index)) { // TODO catch NaN values with vue3
  ...
  goToClosestElement () {
    ...
    if (element) {
      ...
      console.error('Slider:goToClosestElement: No closest element found.', this.drag.x, this.drag.y, this.children)
    }
  }
  ...
  goToNext () {
    const index = Math.min(this.children.length - 1, this.currentIndex + this.options.slidesToScroll) // Math.min(this.children.length - 1, this.currentIndex + this.options.slidesToScroll)
    const element = this.children[index]
  ...
  getClosestElementByCoords (x, y) {
    ...
    // get closest element
    const element = this.children.find((child, childIndex) => { // eslint-disable-line
      ...
      if (this.options.mode === 'horizontal') {
        if (this.options.rtl) {
          if (this.options.center) {
            return bound.x < position.x + (child.clientWidth / 2) + gap && bound.x > position.x - (child.clientWidth / 2) - gap
          } else {
            return bound.x < position.x + (child.clientWidth / 2) - gap && bound.x > position.x - child.clientWidth + gap
          }
        } else {
        ...
        // eslint-disable-next-line no-lonely-if
        if (this.options.center) {
          return bound.y > position.y - (child.clientHeight / 2) - gap && bound.y < position.y + (child.clientHeight / 2) + gap
        } else {
          return bound.y > position.y - (child.clientHeight / 2) - gap && bound.y < position.y + child.clientHeight + gap
        }
      }
    })

    if (!element) console.warn('Slider:getClosestElementByCoords: No closest element found.', bound)
  ...
  onMouseenter () {
    const isInteractable = this.options.mode === 'horizontal' ? (this.options.center || this.element.clientWidth > this.options.container.clientWidth) : (this.options.center || this.element.clientHeight > this.options.container.clientHeight)
  ...
  onStart (event) {
    ...
    // trigger callback
    this.options.onStart && this.options.onStart(event, this)
  }
  ...
  onMove (event) {
    ...
    if (this.isInteracting && isInteractable) {
      ...
      // trigger callback
      this.options.onMove && this.options.onMove(event, this)
    }
  }
  ...
  onEnd (event) {
    ...
    if (this.isInteracting && isInteractable) {
      ...
      // trigger callback
      this.options.onEnd && this.options.onEnd(event, this)
    }
  }

  checkBreakpoints () {
    const breakpointsTests = this.options.responsive.sort((a, b) => b.breakpoint - a.breakpoint).map(responsiveItem => window.matchMedia(`(${responsiveItem.type || 'max'}-width: ${responsiveItem.breakpoint}px)`).matches)
  ...
  onResize (event) {
    ...
    this.currentIndexBeforeResize = this.currentIndexBeforeResize !== null ? this.currentIndexBeforeResize : this.currentIndex
    ...
    requestAnimationFrame(() => {      
      ...
      this.timerResize = setTimeout(() => {
        ...
        // trigger callback
        this.options.onResize && this.options.onResize(event, this)
      }, 150)
    })
  }
  ...
  onWheel (event) {
    if (this.options.center || this.element.clientWidth > this.options.container.clientWidth) {
      ...
      if (this.currentIndexStart === null || this.currentIndexStart === undefined) this.currentIndexStart = this.currentIndex
  ...
  snap (force = false) {
    if (this.options.snap) {
      ...
      const slidesToShowSnapIndecies = Array.from({ length: Math.ceil(this.children.length / slidesToShowSnapSteps) }, (v, i) => i * slidesToShowSnapSteps)
      ...
      if (force || Math.abs(this.dist.x) / this.options.container.clientWidth > this.options.threshold) {
        if (this.options.mode === 'horizontal') {
          // if same index on start or current index is not in snap indecies go to next/prev slide/page
          if (this.currentIndexStart === closestIndecies.current || !slidesToShowSnapIndecies.includes(this.currentIndexStart)) {
            ...
            index = isSubIndex ? closestIndecies.current : (direction > 0 ? closestIndecies.next : closestIndecies.prev) ?? closestIndecies.current

            // console.log('isSubIndex', index, isSubIndex, this.currentIndexStart, closestIndecies)
          }
        } else if (this.options.mode === 'vertical') {
          if (this.currentIndexStart === closestIndecies.current || !slidesToShowSnapIndecies.includes(this.currentIndexStart)) {
            index = Math.sign(this.dist.y) * -1 > 0 ? closestIndecies.next : !slidesToShowSnapIndecies.includes(this.currentIndexStart) ? closestIndecies.current : closestIndecies.prev // Math.max(Math.min(this.options.rtl ? index + Math.sign(this.dist.x) : index - Math.sign(this.dist.x), this.children.length - 1), 0)
          }
        }
      }
  ...
  translate (duration) {
    if (this.options.center || this.element.clientWidth > this.options.container.clientWidth || this.element.clientHeight > this.options.container.clientHeight) {
  ...
  onTranslate () {
    ...
    if (!this.isResizing) this.setCurrentIndex(this.getClosestElementByCoords(this.drag.x, this.drag.y).index)
    ...
    if (this.dotsDragger) {
      ...
      if (!this.isInteracting) {
        this.dotsDragger.drag.x = scale(this.currentIndex / (this.children.length - 1), 0, 1, boundsDragger.boundLeft, boundsDragger.boundRight)
      } else {
        const boundsSlider = this.checkBounds()
        const percentageSliderMoved = scale(this.drag.x, boundsSlider.boundLeft, boundsSlider.boundRight, 1, 0)

        this.dotsDragger.drag.x = scale(percentageSliderMoved, 0, 1, boundsDragger.boundLeft, boundsDragger.boundRight)
      }
    ...
    // trigger callback
    this.options.onTranslate && this.options.onTranslate(this.drag.x, this.drag.y, this)
  }
  ...
  updateStyles () {
    requestAnimationFrame(() => {
      ...
      // set slide styles
      for(const key in styles) this.options.container.parentNode.style.setProperty(key, styles[key])
      ...
      requestAnimationFrame(() => {
        if (this.options.mode === 'horizontal') {
          ...
          const trackHeight = this.children.reduce((acc, child) => (acc += child.getBoundingClientRect().height), 0)
