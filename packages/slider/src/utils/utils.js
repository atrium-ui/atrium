class resizeObserver {
  ...
  createResizeObserver (callback = () => {}, element) {
    if (element) {
      this.instance = new ResizeObserver(entries => { // eslint-disable-line
...
const mergeDefaults = function (config, defaults) {
...
const closest = function (arr, num) {
...
const closestPrevNext = (array, currentValue) => {
  ...
  const closestValue = array.reduce((prev, current) => Math.abs(current - currentValue) <= Math.abs(prev - currentValue) ? current : prev)
