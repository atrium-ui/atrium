...
const normalizeEvent = (event) => {
	if (event.changedTouches?.[0]) event.clientX = event.changedTouches[0].clientX;
	if (event.changedTouches?.[0]) event.clientY = event.changedTouches[0].clientY;

	return event;
};

const createVelocityCalculator = (event) => {
...
class Dragger {
	constructor(element, options, triggerCallback = true) {
		if (element) {
			...
			this.element.style.setProperty(
				'transform',
				`translate3d(${this.drag.x}px, ${this.drag.y}px, 0)`
			);
	...
	unbindEvents(withResize = false) {
		if (this.options.autoResize && this.resizeObserver && withResize)
			this.resizeObserver.destroyResizeObserver();
		if (this.options.wheel)
			this.options.container.removeEventListener('wheel', this.boundedWheelFn);
	...
	checkBounds(x = this.drag.x, y = this.drag.y) {
		...
		// calc offsets
		const offsetX =
			this.options.alignX === 'outer'
				? this.element.clientWidth
				: isSmallerThenDistanceX
				  ? offsetDistanceX > 0
						? this.element.clientWidth - offsetDistanceX
						: offsetDistanceX + this.element.clientWidth
				  : 0;
		const offsetY =
			this.options.alignY === 'outer'
				? this.element.clientHeight
				: isSmallerThenDistanceY
				  ? offsetDistanceY > 0
						? this.element.clientHeight - offsetDistanceY
						: offsetDistanceY + this.element.clientHeight
				  : 0;

		// calc bounds with offsets
		const boundLeft = isOverlappingOffsetX
			? this.options.bounds.minX +
			...
			: this.options.bounds.minX +
			  ...
			  offsetX;
		const boundRight = isOverlappingOffsetX
			? this.options.bounds.maxX -
			  (this.options.container.clientWidth - this.options.offsets.left) +
			  offsetX
			: this.options.bounds.maxX -
			  (this.options.rtl ? this.options.offsets.left : this.options.offsets.right) +
			  offsetX;
		const boundTop = isOverlappingOffsetY
			? this.options.bounds.minY +
			  (this.options.container.clientHeight - this.options.offsets.bottom) -
			  offsetY
			: this.options.bounds.minY + this.options.offsets.top - offsetY;
		const boundBottom = isOverlappingOffsetY
			? this.options.bounds.maxY -
			  (this.options.container.clientHeight - this.options.offsets.top) +
			  offsetY
			: this.options.bounds.maxY - this.options.offsets.bottom + offsetY;

		let xDiff = 0;
		let yDiff = 0;
		let xBound = 0;
		let yBound = 0; // eslint-disable-line
	...
	onMove(event, triggerCallback = true) {
		...
		if (this.isInteracting) {
			...
			const overflowScrollingOffset = this.options.overflowScrolling
				? this.options.overflowScrollingOffset
				: 0;
	...
	onWheel(event, triggerCallback = true) {
		if (
			this.options.prevent(event) ||
			(Math.abs(event.deltaX) > 0 && Math.abs(event.deltaY) < 20)
		) {
			event.preventDefault();
