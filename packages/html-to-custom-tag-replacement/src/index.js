class HtmlToCustomTagReplacement {
  ...
  replaceTags (string = '', fromTag = '', toTag = '', attributes = {}, additionalAttributes = {}) {
    fromTag = fromTag.split('.') // support classes
    ...
    result = result.replace(
      ...
      new RegExp(fromTag[1] ? `(<${fromTag[0]}\\s.*class=["\'](?:.+\\s)?${fromTag.slice(1).join(' ')}(?:\\s.+)?[\'"][^>]*>.*?<\\/${fromTag[0]}>)` : `<${fromTag[0]}(\\/)?.*?>(?:.*<\\/${fromTag[0]}>)?`, 'gmi'), // eslint-disable-line
      function (match) {
        // replace tag
        let resultTag = match.replace(
          new RegExp(`<(\/)?${fromTag[0]}(>)?`, 'gi'), // eslint-disable-line
          function (match, slash, bracket) {
        ...
        resultTag = resultTag.replace(
          ...
          function (match, openingTag, closingTag) {
            const additionalAttributesString = Object.keys(additionalAttributes).map(additionalAttribute => {
