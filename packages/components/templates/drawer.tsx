...
export function Drawer(props: Props, context) {
  return (
    <a-blur
      ...
      class="group/blur fixed left-0 top-0 z-50 block h-full w-full transition-all [&[enabled]]:bg-[#1C1C1C33] [&[enabled]]:backdrop-blur-md"
      onBlur={() => props.onBlur?.()}
    >
      <drawer-track
        vertical
        class="fixed bottom-0 left-1/2 z-50 -translate-x-1/2 translate-y-full overflow-visible opacity-0 transition-all group-[&[enabled]]/blur:block group-[&[enabled]]/blur:translate-y-0 group-[&[enabled]]/blur:opacity-100"
        onClose={() => props.onBlur?.()}
      >
        <div class="w-[calc(100vw-20px)] min-w-[250px] max-w-[500px] rounded-t-lg border border-b-0 border-zinc-700 bg-zinc-800 px-8 py-4">
